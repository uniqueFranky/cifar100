...
class DataParallelTrainer:
    ...
    def __init__(self, config):
        ...
        print(f"模型已分布到 {len(config.gpu_ids)} 个GPU")
    ...
    def save_checkpoint(self, epoch, is_best=False, is_final=False):
        ...
        if is_best:
            path = os.path.join(self.config.save_dir, 'best_model_dp.pth')
            ...
            path = os.path.join(self.config.save_dir, f'checkpoint_dp_epoch_{epoch+1}.pth')
            torch.save(state, path)
